...
:::tip Other Material Sources
See [Materials Documentation](../printing/materials.md) for adding materials manually, from databases, or via bulk import.
:::
...
11. Upload your **STL or 3MF file** and give it a name and description
12. Configure material requirements and process settings
13. Click `Create` to save your part

   :::info File Support
   Printago supports STL, STEP, 3MF, OpenSCAD, and GCODE 3MF files. Regular `.gcode` files are not supported.
   :::

## Step 5: Start Your First Print

14. Find the saved part in the parts library and click the `Print` button
   :::tip Quick Link
...
18. Navigate to `Printing → Printers` and mark printers as `Clear & Ready`:
...
19. **Automatic Processing**: Printago automatically:
...
20. **Monitor Progress**: Use the `Print Queue` to:
